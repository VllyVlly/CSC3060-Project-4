...
uint64_t CacheLevel::get_index(uint64_t addr) {
    ...
    uint64_t temp = addr >> offset_bits;
    temp = temp & ((1ULL << index_bits) - 1);
    return temp;
}
...
uint64_t CacheLevel::get_tag(uint64_t addr) {
    ...
    uint64_t temp = addr >> ((index_bits+offset_bits));
    return temp;
}
...
uint64_t CacheLevel::reconstruct_addr(uint64_t tag, uint64_t index) {
    ...
    uint64_t temp = (tag << ((index_bits+offset_bits))) + (index << (offset_bits));
    return temp;
}
